"""
Clinical Algorithmic Engine & Guideline Rules for ChemoGuard: BSA-Adjusted Antineoplastic Regimen & Lifetime Dose Safety Agent.
Domain: Medical Oncology
Standard: ASCO / NCCN Antineoplastic Regimen Safety
"""
import math
...
class ClinicalDomainEngine:
    GUIDELINE = "ASCO / NCCN Antineoplastic Regimen Safety"
    ...
    def evaluate_primary_index(cls, value: float) -> Optional[Dict[str, Any]]:
        if value > cls.PRIMARY_BASELINE_LIMIT:
            return {
                ...
                "finding": f"Observed value ({value:.2f}) exceeds ASCO / NCCN Antineoplastic Regimen Safety clinical baseline limit ({cls.PRIMARY_BASELINE_LIMIT:.1f}).",
    ...
    def evaluate_biomarker_concordance(cls, status_flag: str, biomarkers: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if "DISCORDANT" in status_upper or "EQUIVOCAL" in status_upper or "MUTANT" in status_upper:
            return {
                ...
                "recommendation": f"Order reflex confirmatory testing per ASCO / NCCN Antineoplastic Regimen Safety clinical recommendations.",
            }
        return None
