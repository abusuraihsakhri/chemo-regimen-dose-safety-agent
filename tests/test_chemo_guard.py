import pytest
from chemo_guard import BSADoseCapAuditorAgent, LifetimeToxicityLimitAgent, PharmacogenomicScreenAgent, ChemoGuardCoordinator, main


def test_sub_agents():
    a1 = BSADoseCapAuditorAgent()
    alerts1 = a1.evaluate({"metric_primary": 35.0})
    assert len(alerts1) == 1

    a2 = LifetimeToxicityLimitAgent()
    alerts2 = a2.evaluate({"critical_flag": True})
    assert len(alerts2) == 1

    a3 = PharmacogenomicScreenAgent()
    alerts3 = a3.evaluate({"status_text": "DISCORDANT_FINDING"})
    assert len(alerts3) == 1


def test_coordinator():
    coord = ChemoGuardCoordinator()
...
def test_domain_registry():
    from chemo_guard import DomainKnowledgeRegistry
